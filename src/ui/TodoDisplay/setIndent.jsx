...
export var Form = React.createClass({
    ...
    mouseUp: function() {
        ...
        if (index != -1) {
            ...
            if (index == 0)
                flag = 1
            else if (todo[index - 1].type == "task" && todo[index].indent > todo[index - 1].indent)
                flag = 2
            else if (todo[index + 1] && todo[index].type == "task" && todo[index].indent < todo[index + 1].indent)
                flag = 3
            console.log(flag)
            if (flag != 0) {
                var newData = update(this.state, {
                    toDoList: {
                        [index]: {
                            indent: {
                                $set: flag == 1 ?
                                    40 : flag == 2 ?
                                    todo[index - 1].indent : todo[index + 1].indent
                            }
                        }
                    },
                })
